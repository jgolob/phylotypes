...
Author: Generated based on existing phylotypes implementation
License: GPL-3.0+
"""
...
def LWR_Filter(mat: np.ndarray, threshold: float) -> np.ndarray:
    """
...
class Phylotypes:
    ...
    def _load_tree(self) -> None:
        ...
        re_sepp_tree = re.compile(
            r'(|\w+):(?P<edgelen>(\d+\.\d+)(|e[-+]\d+))\[(?P<edgeid>\d+)\]'
        )
        
        def normalized_edges(m: re.Match) -> str:
    ...
    def _pregroup_by_lwr(self, pd_threshold: float, lwr_overlap: float) -> None:
        ...
        sv_to_group = [
            ...
            sorted([
                (sv, len(svp))
                for sv, svp in self.sv_nodes.items()
            ], key=lambda v: v[1])
        ]
        
        sv_groups: List[Set[str]] = []
        ...
        while len(sv_to_group) > 0:
            ...
            group_svs.update({
                sv
                for (sv, m) in zip(sv_to_group, group_mask.astype(bool))
                if m
            })
            ...
            logging.debug(f'{len(sv_to_group)} SVs remain to pregroup, '
                         f'with {len(group_node_ids)} nodes at this step')

        logging.info("Done pre-grouping features into {} groups, "
                    "of which the largest is {} items".format(
                        len(sv_groups),
...
def main() -> None:
    ...
    phylotypes.group_features(
        args.lwr_overlap, 
        args.threshold_pd, 
        no_dl=args.no_distal_length
    )
